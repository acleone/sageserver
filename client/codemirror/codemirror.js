...
var CodeMirror = (function(){
  ...
  function CodeMirror(place, options) {
    ...
    if (options.height == 'dynamic') {
      frame.scrolling = 'no';
      this.updateHeight = function () {
        /*var vmargin = 2 * this.frame.offsetTop,
            body = this.win.document.body,
            lineHeight = null;
        for (var span = body.firstChild, sawBR = false; span; span = span.nextSibling) {
          if (this.win.isSpan(span) && span.offsetHeight) {
            lineHeight = span.offsetHeight;
            if (!sawBR) vmargin = 2 * (this.frame.offsetTop + span.offsetTop + body.offsetTop + (internetExplorer ? 10 : 0));
            break;
          }
        }
        if (lineHeight) {
          this.wrapping.style.height = Math.max(vmargin + lineHeight * (body.getElementsByTagName("BR").length),
                                                this.options.minHeight) + "px";
        }*/
        var body = this.win.document.body;
        var brs = body.getElementsByTagName('BR');
        var ll_not_empty = true;
        try {
            ll_not_empty = this.lineContent(this.lastLine()).length;
        } catch (e) {}
        var nlines = (ll_not_empty)? brs.length + 1 : brs.length;
        this.wrapping.style.height = (0.3 + Math.max(nlines, 1.2)) + 'em';
      };
    }
